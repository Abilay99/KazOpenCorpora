...
ob = corporaDB()
mylist = ob.Corpora_column_morph_emty()
length = len(mylist)
for i in range(length):
    command = "cd /media/gpu2/59f87a06-90bf-49c9-a6c0-34f26ab5287c/SEproject/Source/apertium-kaz\n echo '%s'| apertium -n -d. kaz-tagger"%mylist[i]['text']
    apertium = out(command)
    eadb = EditedApertium_DB(text = mylist[i]['text'], apertium = apertium)
    editedapertium = str(eadb)
    outtexts = str(outtexts_DB(aptext = editedapertium))
    train = str(train_DB(outtexts=outtexts))
    ob.KESTENI_JANARTU(bagan_aty='morphanaliz', bagan_mani=editedapertium, bagan_id=mylist[i]['id'])
    ob.KESTENI_JANARTU(bagan_aty='outtext', bagan_mani=outtexts, bagan_id=mylist[i]['id'])
    ob.KESTENI_JANARTU(bagan_aty='lemmas', bagan_mani=train, bagan_id=mylist[i]['id'])
